...
def save_result(study_direction, full_name, stu_numb, group_name):
    try:
        conn = pymysql.connect(host='localhost',
                               user='root',
                               password='root',
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO " + NameDB.namedb.get_name_tb() + f" (study_direction, FullName, StuNumb, GroupName) VALUES (%s, %s, %s, %s)",
            (str(study_direction), str(full_name), int(stu_numb), str(group_name)))
        conn.commit()
